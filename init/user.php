...
class User{
    ...
    public static function get_web_addess(){
        return "https://rfhubglobal.com/";
    }
    public static function get_company_name(){
        return "RFHubGlobal";
    }
    public static function get_mailer_host(){
        return "server122.web-hosting.com";
    }
    ...
    public static function generate_hash($unique=0){
        ...
        $identify = 'RFHubGlobal-'.$current_day.'-'.$current_month.'-'.$current_year.'-'.$current_hour.'-'.$alpha.'-'.$current_min.'-'.$delta.'-'.$current_sec.'-'.$zigma;
        return $identify;
    }
    ...
    public function forgotten_password_message($ref_id="", $email=""){
		...
		$message = '
        <img src="cid:logoimg" width="182" height="100"><br>
        <h1><b>RFHubGlobal</b></h1>
        '.'

        <p>Please click this link to set new password:</p>
        '
        .self::get_web_addess().'/login?recorver='.$identify.''.'<br>
        '.'
    ...
    public function verification_mail_message($pass, $email, $ref_id){
        ...
        '
        .self::get_web_addess().'?/login?confirm='.$identify.''.'
        '.'</p>
        <p>
    ...
    public function send_payment_info($dollar_amount="", $user_balance=""){
        ...
        RFHubGlobal Balance: '.$user_balance.''.'</b><br>
        ------------------------
        '.'
    ...
    public function helpdesk_mail_message($msg){
        $message = '
        <img src="cid:logoimg" width="182" height="100"><br>
        <h1>RFHubGlobal</h1>
        '.'
    ...
    public static function register_user($ref_id="", $fname="", $lname="", $email="", $phone="", $pass=""){
        global $db_init;
        $epass = md5($pass);
        
        $query = "INSERT INTO users SET status='1', created=NOW(), ref_id='{$ref_id}', fname='{$fname}', lname='{$lname}', email='{$email}', phone='{$phone}', pass='{$epass}'";
