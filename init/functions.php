...
function send_contact($email, $message, $phone){
    return User::find_sql("INSERT INTO contact SET email='{$email}', message='{$message}', phone='{$phone}'");
}

// function send_contact($email, $pass){
//     return User::find_sql("INSERT INTO contact SET email='{$email}', message='{$message}', phone='{$phone}'");
// }

function swiss_allert($type, $message, $loc){
    if(empty($loc)){
        echo "<script>
            $(function(){
                setTimeout(function(){
